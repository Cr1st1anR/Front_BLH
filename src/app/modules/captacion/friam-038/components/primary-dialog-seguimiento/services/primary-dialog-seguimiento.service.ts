...
export class PrimaryDialogSeguimientoService {
  ...
  private transformarVisitasParaTabla(visitas: VisitaSeguimiento[]): VisitaTabla[] {
    // ✅ CORRECCIÓN: Ordenar por fecha ascendente para numerar correctamente
    ...
    return visitasOrdenadas.map((visita, index) => ({
      id_visita: visita.id,
      no_visita: index + 1, // ✅ Número basado en orden cronológico
  ...
  // Usar valores directos sin new Date() para evitar problemas de zona horaria
